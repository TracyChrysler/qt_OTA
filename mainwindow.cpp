...
void MainWindow::on_startButton_clicked()
{
    ...
    while (timer.elapsed() < 60000) { // 1 minute timeout
        serial->write(cmdStart);

        if (serial->waitForBytesWritten(100)) { // wait 100 milliseconds for writing
        ...
        if (serial->waitForReadyRead(100)) { // wait 100 milliseconds for response
            ...
            qDebug() << "No response received. Retrying...";
        }

        QThread::msleep(100); // wait 100 milliseconds before retrying
    }

    if (!upgradeStarted) {
        QMessageBox::critical(this, tr("错误"), tr("设备未响应开始命令！升级失败！"));
        return;
    }

    // 开始发送数据包
    while (currentIndex < firmwareData.size()) {
        ...
        if (serial->waitForReadyRead(100)) { // wait 100 milliseconds for response
            ...
            if (!response.isEmpty() && static_cast<uint16_t>(response[0]) == packetId) {
                ...
                continue; // retry sending the current data block
            }
        } else {
